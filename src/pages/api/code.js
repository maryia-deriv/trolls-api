...
function updatePlaygroundWithRequestAndResponse() {
    ...
    appendToConsoleAndScrollIntoView(
        '<pre class="req">' + jsonToPretty(json) + "</pre>" + '<div class="progress"></div>'
    );
    sendRequest(json);
}
...
function getFormattedJsonStr(json) {
    ...
    return (json.error ? '<pre class="error">' : "<pre>") + jsonToPretty(json) + "</pre>";
}
...
function applicationsTableRow(application) {
    ...
    $("button[id=" + application.app_id + '][class="update"]').on("click", function (e) {
        ...
        if (form_title) {
            form_title.innerText = form_title.innerText.replace("Register", "Update");
        }
        $("#placeholder_app_id")
            .text(" " + e.target.id + " ")
...
function sendApplicationRequest(app_id) {
    var request = app_id ? { app_update: app_id, scopes: [] } : { app_register: 1, scopes: [] };
...
function isProduction(url) {
    return url && (/(api\.deriv\.com|binary\.sx)/i.test(url) || /(staging-api\.deriv\.com)/i.test(url));
}
...
function endpointNotification() {
    ...
    if (end_note) {
        ...
        if (server !== DEFAULT_API_URL || brand !== DEFAULT_BRAND) {
            end_note.innerHTML =
                'The server <a href="/endpoint/">endpoint</a> is: ' + server + " (Brand: " + brand + ")";
...
function addEventListeners() {
    ...
    CODE_SAMPLES.forEach(el =>
        $(`#demo-${el}`).on("change", function () {
